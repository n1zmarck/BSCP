...
app = Flask(__name__)
app.secret_key = "secret_handshake_key"

# Configuration from launch arguments
PORT = int(sys.argv[1]) if len(sys.argv) > 1 else 5000
DOMAIN = sys.argv[2] if len(sys.argv) > 2 else f"localhost:{PORT}"

# Database Setup
app.config['SQLALCHEMY_DATABASE_URI'] = f'sqlite:///database_{PORT}.db'
db = SQLAlchemy(app)
